...
def install_page_html():
    prefill = request.args.get('zip_url', '')
    return f"""<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"/>
...
<style>
{COMMON_CSS}
.install-box{{max-width:600px;background:var(--card);border:1px solid var(--border);
  border-radius:3px;padding:28px;margin:0 auto}}
.install-box h2{{font-family:var(--display);font-size:15px;font-weight:700;margin-bottom:20px}}
.form-row{{margin-bottom:14px}}
.form-row label{{display:block;font-size:10px;color:var(--dim);letter-spacing:.1em;margin-bottom:5px}}
.form-row input{{width:100%}}
.drop-zone{{border:2px dashed var(--border);border-radius:3px;padding:32px;text-align:center;
  color:var(--dim);font-size:12px;cursor:pointer;transition:border-color .15s;margin-bottom:14px}}
.drop-zone:hover,.drop-zone.drag{{border-color:var(--amber);color:var(--amber)}}
.drop-zone input{{display:none}}
.progress{{background:var(--panel);border-radius:2px;height:4px;overflow:hidden;margin:10px 0}}
...
<main>
  ...
  <div class="subtitle">Upload a generated module zip to add it to MasterChief</div>
  <div class="install-box">
    <h2>UPLOAD MODULE ZIP</h2>
    ...
    <div class="drop-zone" id="drop-zone"
      ...
      <div style="font-size:10px;margin-top:6px;color:var(--dim)">
        Upload to: masterchief/addons/modules/
      </div>
    </div>

    <div id="progress-wrap" style="display:none">
      <div class="progress"><div class="progress-bar" id="pbar"></div></div>
      <div id="progress-msg" style="font-size:11px;color:var(--mid);text-align:center"></div>
    </div>

    <div id="result"></div>

    <div style="margin-top:20px;border-top:1px solid var(--border);padding-top:16px">
      <div style="font-size:10px;color:var(--dim);letter-spacing:.1em;margin-bottom:10px">PREVIOUSLY GENERATED</div>
      <div id="generated-list" style="font-size:12px;color:var(--mid)">Loading…</div>
    </div>
  </div>
</main>
<script>
...
async function handleFile(file) {{
  ...
  try {{
    const r = await fetch(PROXY_BASE + '/api/upload', {{
      method: 'POST',
      body: fd,
    }});
    ...
    if (d.success) {{
      ...
      showResult('success', `✅ Module "${{d.slug}}" installed. Restart MasterChief or use the addon manager to activate it.`);
    }} else {{
...
def api_upload():
    """Handle zip upload — install to addons/modules/."""
    f = request.files.get('file')
    if not f or not f.filename.endswith('.zip'):
        return jsonify({'error': 'No zip file provided'}), 400

    # Determine addons dir — 4 levels up from addon folder
    addons_dir = BASE_DIR.parent.parent  # addons/modules/
    if not addons_dir.exists():
        return jsonify({'error': f'Addons dir not found: {addons_dir}'}), 500

    try:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_zip = Path(tmp) / f.filename
            f.save(str(tmp_zip))

            with zipfile.ZipFile(tmp_zip, 'r') as zf:
                names = zf.namelist()
                # Expect zip to contain a single top-level folder
                ...
                for name in names:
                    if '..' in name or name.startswith('/'):
                        return jsonify({'error': f'Unsafe path in zip: {name}'}), 400

                dest = addons_dir / slug
                dest.mkdir(parents=True, exist_ok=True)
                zf.extractall(str(addons_dir))

        return jsonify({'success': True, 'slug': slug, 'path': str(addons_dir / slug)})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
